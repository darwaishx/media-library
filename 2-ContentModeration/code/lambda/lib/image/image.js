 ...
 let image = (function() {
    ...
    */
     ...
     */
     ...
     /**
      * Recognizes moderation labels in an image
      * @param {JSON} image_info - information about the image to be processed
      * @param {getLabels~callback} cb - The callback that handles the response.
      */

      image.prototype.getModerationLabels = function(image_info, cb){
         console.log('Executing image moderation label detection');
         ...
         let rek_params = {
          ...
          rekognition.detectModerationLabels(rek_params, function(err, data) {
             if (err) {
                 ...
                 * Build label metadata array
                 */
                 let moderation_labels = [];
                 for (var l in data.ModerationLabels) {
                    if (data.ModerationLabels[l].Confidence >= confidence_score) {
                        moderation_labels.push(data.ModerationLabels[l].Name.toLowerCase());
                    }
                 }

                 let moderation_label_key = ['private',image_info.owner_id,'media',image_info.object_id,'results','moderation_labels.json'].join('/');

                 let s3_params = {
                     Bucket: s3Bucket,
                     Key: moderation_label_key,
                     Body: JSON.stringify(data),
                     ContentType: 'application/json'
                 };
                 ...
                 upload.respond(s3_params, function(err, response) {
                     if (err){
                       ...
                       return cb(err, null);
                         ...
                         console.log(response);
                         let moderation_label_response = {'key': moderation_label_key, 'moderation_labels': moderation_labels, 'status': 'COMPLETE'};
                         return cb(null,moderation_label_response);
                     }
                 });
             }
          });
       ...
       */
        ...
        */
         ...
         */
         ...
         image.prototype.getDetectText = function(image_info, cb){
            console.log('Executing image text detection');
                ...
                if (err) {
                    ...
                    */
                    ...
                    for (var l in data.TextDetections) {
                       if (data.TextDetections[l].Type === 'LINE' && data.TextDetections[l].Confidence >= confidence_score) {
                           text_detections.push(data.TextDetections[l].DetectedText.toLowerCase());
                       }
                    }

                    let text_detections_key = ['private',image_info.owner_id,'media',image_info.object_id,'results','detect_text.json'].join('/');

                    let s3_params = {
                        Bucket: s3Bucket,
                        Key: text_detections_key,
                        Body: JSON.stringify(data),
                        ContentType: 'application/json'
                    };
                    ...
                    upload.respond(s3_params, function(err, response) {
                        if (err){
                          ...
                          return cb(err, null);
                            ...
                            console.log(response);
                            let text_detections_response = {'key': text_detections_key, 'detect_text': text_detections, 'status': 'COMPLETE'};
                            return cb(null,text_detections_response);
                        }
                    });
                }
             });
